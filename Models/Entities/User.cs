...
public class User
{
    ...
    public int GetRemainingAccessDays()
    {
        var now = DateTime.UtcNow;

        var trialDays = TrialEndDate != null
            ? (TrialEndDate.Value - now).Days
            : 0;

        var subscriptionDays = SubscriptionExpiryDate != null
            ? (SubscriptionExpiryDate.Value - now).Days
            : 0;

        var remaining = Math.Max(trialDays, subscriptionDays);
